...
namespace GroentenWinkel.Api.Controllers
{
    ...
    public class MandjeItemController : ControllerBase
    {
        ...
        {
            double totaal = Math.Round(MandjeItems.Sum(i => i.TotaalPrijs), 2);
        ...
        [HttpPut("changeAantal/{id}")]

        public IActionResult ChangeAantal(int id, [FromBody] double aantal)
        {
            var item = MandjeItems.FirstOrDefault(x => x.Id == id);
            if (item != null && aantal >= 0)
            {
                item.Stuk = aantal;
                item.TotaalPrijs = Math.Round(aantal * item.Prijs, 2);
                return base.Ok(MandjeItems);
            }
            return base.BadRequest(MandjeItems);
        }
    }
